...
using IngredientsSearchResult = System.Collections.Generic.Dictionary<int, WannaEat.FoodService.MMenu.Models.SingleIngredientSearchResult>;
...
public class ParallelIngredientSearcher: IIngredientSearcher
{
    ...
    public async Task<IEnumerable<string>> SearchIdsForIngredients(IEnumerable<Ingredient> ingredients, CancellationToken token)
    {
    ...
    private async Task<IEnumerable<SingleIngredientSearchResult>> SearchIdsForIngredientAsync(Ingredient ingredient, CancellationToken token)
    {
        try
        {
            ...
            var searchResult = await response
                                    .Content
                                    .ReadFromJsonAsync<Dictionary<int, SingleIngredientSearchResult>>(cancellationToken: token);
        ...
        {
            _logger.LogWarning(request, "Could not request ids for ingredient '{IngredientName}'. Error while sending request", ingredient.Name);
            return Enumerable.Empty<SingleIngredientSearchResult>();
        }
        catch (TaskCanceledException cancelled)
        {
            _logger.LogWarning(cancelled, "Could not get ids for ingredient '{IngredientName}'. Task was cancelled", ingredient.Name);
            return Enumerable.Empty<SingleIngredientSearchResult>();
        }
